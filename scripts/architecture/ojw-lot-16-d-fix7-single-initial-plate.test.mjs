...
assert.match(scene, /const dayMounted = targetMode === "day" \|\| preparedMode === "day" \|\| loadedModes\.has\("day"\)/);
assert.match(scene, /const nightMounted = targetMode === "night" \|\| preparedMode === "night" \|\| loadedModes\.has\("night"\)/);
...
assert.match(hero, /<TreeScene mode=\{resolvedMode\} preference=\{preference\} preparedMode=\{preparedMode\}/);
...
assert.equal((scene.match(/3840w/g) ?? []).length, 4);
