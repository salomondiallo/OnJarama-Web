...
for (const field of ["registered", "domainStatuses", "editoriallyApproved", "publiclyAvailable", "routePrefix"]) {
  assert.match(types, new RegExp(`\\b${field}:`, "u"), `LocaleConfiguration must expose ${field}.`);
}
assert.match(config, /fr: \{[^\n]*domainStatuses: \{ \.\.\.missingDomainStatuses, global: "CANONICAL_APPROVED", home: "CANONICAL_APPROVED", foundation: "PARTIAL" \}[^\n]*editoriallyApproved: true[^\n]*publiclyAvailable: true[^\n]*routePrefix: ""/u, "French migrated domains must preserve their honest coverage while the locale stays approved and public.");
